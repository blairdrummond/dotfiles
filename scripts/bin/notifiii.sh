...
inotifywait -m --exclude "/in$" --format "%w %f" -e modify -r "$i" | grep  --line-buffered -v  "^/home/blair/irc/irc.freenode.net/ out$" | \
	while read -r p f; do
		# ignore notifications files other than 'out'
		[ "$f" != "out" ] && continue

		## ii is no longer running
		tmux list-sessions | grep -q "^IRC" || exit
